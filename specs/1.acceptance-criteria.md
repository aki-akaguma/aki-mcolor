...
- **AC8:** Where the `--magenta <exp>` option is used, **the system shall** color all substrings matching the regular expression `<exp>` in magenta.
...
- **AC19:** Where the `AKI_MCOLOR_COLOR_SEQ_MAGENTA_ST` environment variable is set, **the system shall** use its value as the start sequence for magenta coloring.
